...
3. **Write `plan/iterations/<NN.x>-<slug>/PROMPT.md`.** Lighter than a planned one is fine (goal, scope, out of scope, done-when, deliverables incl. the sign-off stop) — but the Explore findings go in. Use the PROMPT template (`${CLAUDE_PLUGIN_ROOT}/skills/klaus/references/templates/PROMPT-template.md`) as the skeleton and cut what doesn't apply. Keep two things even in the light version: the `Kind` (`feature`/`spike`) and an `auto`/`manual` tag on every done-when criterion — a bug fix in particular should get a regression test (`auto`) that fails before the fix.
