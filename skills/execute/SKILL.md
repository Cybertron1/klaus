...
Argument: iteration number (`04`, `10.a`).
...
1. Spawn the **executor** (`klaus:klaus-executor`, foreground): iteration folder path, resume flag if applicable. It implements per PROMPT.md, verifies every acceptance criterion itself, and returns its fixed-format summary. It will not write REPORT.md.
2. Spawn the **reviewer** (`klaus:klaus-reviewer`, foreground): iteration folder path + the executor's files-touched list. It adversarially re-verifies every criterion and scope discipline.
